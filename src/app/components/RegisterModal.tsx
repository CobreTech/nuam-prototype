...
export default function RegisterModal({ open, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Admin-only: use server API with Admin SDK to create user without affecting current session
      ...
      // Firestore profile is created on server; we additionally ensure client cache writes if needed
      setSuccess(true);
      setTimeout(() => {
        onClose();
        resetState();
      ...
      if (typeof message === "string") {
        if (message.includes("email-already-in-use")) friendly = "El correo ya está en uso.";
        else if (message.includes("invalid-email")) friendly = "Correo inválido.";
        else if (message.includes("weak-password")) friendly = "La contraseña es muy débil (min 6).";
        else if (message.includes("Administrador")) friendly = message;
      }
      setError(friendly);
    } finally {
      setLoading(false);
